      ...
      <div className="flex-between w-full max-3xl:max-w-screen-xl body-text !text-xm py-3 px-4">
        ...
        <div className="max-lg:hidden flex-center gap-x-3">
          {NAV_ACCOUNT.map(item =>
            <div className="flex-center text-gradient_blue-purple font-bold pb-1" key={item.key}>
              <Link className="hover flex-center gap-x-1" href={item.slug}>
                <Image className="white-svg object-contain" alt="logo website" src={item.icon} priority={false} width={20} height={18} />
                {item.title}
              </Link>
            </div>
          )}
